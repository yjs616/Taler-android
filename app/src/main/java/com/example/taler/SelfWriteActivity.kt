        ...
        bookService.getBooksByName(getString(R.string.interparkAPIKey), keyword,2,10,"salesPoint")
            .enqueue(object : Callback<SearchBookDto> {
                ...
                override fun onResponse(
                    ...
                    // 받은 응답의 바디가 채워져 있는 경우만 진행;
                    response.body()?.let { searchBookDto ->
                        Log.d(M_TAG, searchBookDto.toString())

                        searchBookDto.books.forEach { book ->
                            Log.d(M_TAG, "${book.title} ${book.coverLargeUrl}")
                        }

                        // 새 리스트로 갱신
                        searchBookDto.books as MutableList<Book>
                        var i =0
                        Glide.with(binding.bigBookIv.context)
                            .load(searchBookDto.books[i].coverSmallUrl)
                            .into(binding.bigBookIv)

                        /*binding.bigBookIv.setOnClickListener{
                            i++
                            Glide.with(binding.bigBookIv.context)
                                .load(searchBookDto.books[i].coverSmallUrl)
                                .into(binding.bigBookIv)
